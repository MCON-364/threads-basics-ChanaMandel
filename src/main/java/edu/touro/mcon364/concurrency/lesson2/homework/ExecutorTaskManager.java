...
public class ExecutorTaskManager {
     ...
     *
     * You would use a CountDownLatch initialized to the batch size. Each work calls the
     * countDown() when it finishes, and the coordinating thread calls await()
     * to block until all the workers have checked in. This is best because once the
     * batch is done, you create a fresh latch for the next one.
     * ──────────────────────────────────────────────────────────────────────*/
    ...
    private void recordCompleted(Task task) {
        //ArrayList is not thread safe so without a lock two worker threads could try to
        //add to it at the same time to the same spot in the ArrayList and data would be
        //lost, a lock makes sure only one thread can add at a time.
        // TODO: implement
        lock.lock();
        try {
